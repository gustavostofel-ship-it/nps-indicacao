...
// "Gustavo Stofel Silva" -> "Gustavo S." — usado pra mostrar responsável sem
// ocupar o espaço do nome completo (ex: quem pegou uma pendência pra si).
export function nomeComInicial(nomeCompleto: string) {
  const partes = nomeCompleto.trim().split(/\s+/).filter(Boolean);
  if (partes.length <= 1) return partes[0] || '';
  return `${partes[0]} ${partes[partes.length - 1][0].toUpperCase()}.`;
}

// "Gustavo Stofel Silva" -> "GS" — versão minimizada (ex: badge de avatar
// num espaço bem apertado), primeira letra do primeiro + último nome.
export function iniciaisNome(nomeCompleto: string) {
  const partes = nomeCompleto.trim().split(/\s+/).filter(Boolean);
