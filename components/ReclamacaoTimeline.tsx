...
import {
  ReclamacaoEvento,
  buscarEventosReclamacao,
  descreverEventoReclamacao,
  registrarObservacaoReclamacao,
} from '@/lib/reclamacoes';
...
const DOT_CLASSES: Record<string, string> = {
  ...
  descricao_alterada: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400',
};

type Usuario = { id: string; nome: string };

export default function ReclamacaoTimeline({
  supabase,
  reclamacaoId,
  usuarios = [],
  currentUserId,
  ...
  const [eventos, setEventos] = useState<ReclamacaoEvento[]>([]);
  ...
  const carregar = async () => {
    setLoading(true);
    const data = await buscarEventosReclamacao(supabase, reclamacaoId);
    setEventos(data);
    setLoading(false);
  };
  ...
  const handleEnviar = async () => {
    ...
    const error = await registrarObservacaoReclamacao(supabase, reclamacaoId, novaNota.trim(), currentUserId);
    if (error) {
      ...
      <div>
        ...
        <div className="flex gap-2">
          <textarea
            ...
            rows={2}
            placeholder="Registre uma ligação, retorno da liderança ou anotação sobre o andamento..."
          />
          <button
                ...
                <li key={ev.id} className="flex gap-3 relative">
                  ...
                  <div className={`flex-1 ${isLast ? 'pb-0' : 'pb-4 mb-4 border-b border-slate-100 dark:border-slate-700/60'}`}>
                    <div className="flex justify-between items-start gap-2 flex-wrap">
                      <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                        {ev.tipo === 'responsavel_alterado'
                          ? (ev.valor_novo
                              ? `Encaminhado: ${ev.valor_anterior ? getNome(ev.valor_anterior) : 'Ninguém'} → ${getNome(ev.valor_novo)}`
                              : `Responsável removido: ${getNome(ev.valor_anterior)}`)
                          : descreverEventoReclamacao(ev)}
                      </span>
                    ...
                    {ev.tipo === 'descricao_alterada' && (
                      <div className="mt-2 bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-700/60 rounded-lg px-3 py-2 text-sm text-slate-600 dark:text-slate-300 whitespace-pre-wrap">
                        {ev.valor_novo || <span className="italic text-slate-400 dark:text-slate-500">(detalhes removidos)</span>}
                      </div>
                    )}
                  </div>
                </li>
