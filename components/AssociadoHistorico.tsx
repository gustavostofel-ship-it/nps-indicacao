...
export default function AssociadoHistorico({
  ...
  if (loading) {
    return <div className="h-8 bg-slate-100 rounded-lg animate-pulse" />;
  }

  if (eventos.length === 0) {
    return <p className="text-xs text-slate-400 italic">Nenhum evento registrado ainda.</p>;
  }
    ...
    <ul className="space-y-2">
      {eventos.map((ev) => (
        <li key={ev.id} className="flex items-start gap-2 text-xs">
          ...
          <div>
            <span className="text-slate-600">{descreverEventoAssociado(ev)}</span>
